function About() {
  return (
    <div id="about_me_container">
      <div id="photo_container">
        <img
          id="photo"
          src="https://portfolio-ian-ennis.s3.us-west-2.amazonaws.com/IanEnnis.png"
          alt="picture_of_ian"
        />
      </div>
      <div>
        <p>
          Welcome. I am a front end web developer with a specialty in ReactJS,
          alongside skills in TypeScript, JavaScript, HTML, Vanilla CSS,
          with automated testing, and API development using Ruby on Rails. Additional
          skills may be viewed under Stack(s).
        </p>
        <p>
          I once designed and enhanced aesthetics of highly-visible outdoor
          public spaces, and have carried my design pallet forward to create
          visually-pleasing and exciting user interfaces. I bring experience
          collaborating on highly motivated teams that are dedicated to solving
          problems and delivering quality.
        </p>
        <p>
          I thoroughly enjoy coding, building both front end and full stack web
          applications, and continually explore the confluence of programming,
          user experience, and positive intentions.
        </p>
        <p>Come have a look at my projects, and get in touch!</p>
      </div>
    </div>
  );
}

export default About;
